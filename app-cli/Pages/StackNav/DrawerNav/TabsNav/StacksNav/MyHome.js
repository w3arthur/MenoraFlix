...
<PageContainer> 
  <ScrollView contentContainerStyle={{/*flex:1,*/ minHeight: WINDOW_HEIGHT*0.825}}>
    ...
    <Input2 ref={searchRef} onChangeText={handleSearchDelay} leftIcon={<Box><NativeButton2 onPress={async() => await handleRestart()}><MaterialCommunityIcons name="restart" size={34} style={{width: 34, height: 34}} color='#2D2D2D' /></NativeButton2></Box>} />
    <Box style={{  flex: 1, display: 'flex', flexDirection: 'column', minHeight: WINDOW_HEIGHT*0.825}}> 
      ...
      <Box style={{  flex: 2.1, paddingTop: 15, minHeight: 190 }}>
        ...
        <Carousel data={topMovies} onPress={(movie) => { handleSetSelectedMovie(movie) }} 
          additionalBox={ (movie) => (<NativeButton2 onPress={() => {  handleStarPress(movie) }}><Center  style={{minHeight: 30, backgroundColor: '#2D2D2D', minHeight: 26,borderTopWidth: 1, borderColor: '#2F2F2F', borderBottomRightRadius: 2, borderBottomLeftRadius: 2}}>
          <Box><Entypo name="star" size={24} style={{height: 24, width: 24}} color={ favorites?.find(x => x.id === movie.id)? '#E40412' : '#f1f1f1' } /></Box>
          </Center></NativeButton2>) }/>
      </Box>
